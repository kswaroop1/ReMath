...
void main() {
  test('each offered goal has teachable skills and valid prerequisites', () {
    ...
    expect(curriculum.goals.map((g) => g.id), ['number-fluency', 'proportions']);
  ...
  test('division stays exact and deterministic at every difficulty', () {
    ...
    for (var level = 0; level < 3; level++) {
      for (var seed = 0; seed < 100; seed++) {
        ...
        final values = RegExp(r'\d+').allMatches(q.prompt)
            .map((m) => int.parse(m.group(0)!)).toList();
        expect(values[1], greaterThan(0));
        expect(values[0] % values[1], 0);
        expect(q.mark('${values[0] ~/ values[1]}').verdict, AnswerVerdict.correct);
  ...
  test('fraction answers accept equivalent fractions and reject zero denominator', () {
    final q = NumberCurriculum().question('number.fractions', 0, 7, 0);
    final fraction = q.answer.split('/').map(int.parse).toList();
    expect(q.mark('${fraction[0] * 2}/${fraction[1] * 2}').verdict,
    ...
    for (final skill in curriculum.skills) {
      for (var level = 0; level < 3; level++) {
        for (var seed = 0; seed < 20; seed++) {
          ...
          expect(q.choices.where((c) => q.mark(c.value).verdict == AnswerVerdict.correct).length, 1);
  ...
  test('question generation rejects invalid identities and levels', () {
    ...
    expect(() => c.question('arithmetic.division', -1, 0, 0), throwsArgumentError);
    expect(() => c.question('arithmetic.division', 3, 0, 0), throwsArgumentError);
    expect(() => c.question('arithmetic.division', 0, 0, -1), throwsArgumentError);
  });
}
