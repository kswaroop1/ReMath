...
void main() {
  ...
  test(
    'seeded arithmetic generation preserves every learner-facing invariant',
    () {
      ...
      for (final operation in ArithmeticOperation.values) {
        ...
        for (var seed = 0; seed < 200; seed++) {
          for (var index = 0; index < 50; index++) {
            ...
            expect(
              question.right,
              inInclusiveRange(0, template.maximumOperand),
            );
            expect(
              question.left,
              greaterThanOrEqualTo(template.minimumOperand),
            );
            expect(
              question.right,
              greaterThanOrEqualTo(template.minimumOperand),
